...
import { saveGuestRoutine, type GuestRoutineInput } from "@/lib/guest/storage";
...
type RoutineDetailResponse = {
  ...
  description: string | null;
  blocks: Array<{
    type: "single" | "bi_series" | "tri_series";
    exercises: Array<{
      exerciseId: string;
...
async function cloneRoutineToGuest(routineId: string): Promise<{ ok: true; id: string } | { error: string }> {
  const res = await fetch(`/api/community/${routineId}`);
  if (!res.ok) return { error: "No pudimos cargar esta rutina." };
  const detail: RoutineDetailResponse = await res.json();

  const input: GuestRoutineInput = {
    name: detail.name,
    ...
    blocks: detail.blocks.map((b) => ({
      ...
      exercises: b.exercises.map((ex) => ({
        ...
        exerciseImage: ex.exercise?.image ?? null,
  ...
  return saveGuestRoutine(input);
}

export function ComunidadPublicBrowse({ isSignedIn }: { isSignedIn: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {loading ? (
        ...
        <p className="rounded-2xl border border-[#2a2f37] bg-[#1c2026] px-6 py-12 text-center text-[#9099a3]">
          {q ? "No encontramos a nadie con ese nombre." : "Todavía no hay rutinas públicas."}
        </p>
      ) : (
        <>
          {people.length > 0 && (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {items.map((routine) => (
                ...
                >
                  ...
                  <FollowButton routineId={routine.id} isSignedIn={isSignedIn} />
                </div>
              ))}
            </div>
          )}
...
function FollowButton({ routineId, isSignedIn }: { routineId: string; isSignedIn: boolean }) {
  const router = useRouter();
  const [status, setStatus] = useState<"idle" | "saving" | "error">("idle");

  if (isSignedIn) {
    return (
      <form action={followRoutineAction.bind(null, routineId)}>
        <button
          type="submit"
  ...
  async function followAsGuest() {
    setStatus("saving");
    const result = await cloneRoutineToGuest(routineId);
    if ("error" in result) {
      setStatus("error");
      return;
    }
    router.push(`/invitado/rutinas/${result.id}`);
  }
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <button
        type="button"
        onClick={followAsGuest}
        disabled={status === "saving"}
        className="w-full rounded-[10px] border border-[#2a2f37] px-4 py-2 text-sm font-bold text-[#f1f3f4] transition-colors hover:border-[#4ade80] disabled:opacity-60"
      >
        {status === "saving" ? "Copiando…" : "Seguir esta rutina"}
      </button>
      {status === "error" && (
        <span className="text-xs font-semibold text-[#f87171]">
          No pudimos copiarla. Intenta de nuevo.
        </span>
      )}
    </div>
  );
