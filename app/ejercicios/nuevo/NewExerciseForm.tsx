...
export default function NewExerciseForm({ onCreated, onSavingChange }: {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!gif || saving || converting) return;
    ...
    payload.set("gif", gif, "exercise.gif");
    setSaving(true); setError("");
    onSavingChange?.(true);
    try {
      const response = await fetch("/api/exercises/create", { method: "POST", body: payload });
  ...
  if (createdId) return <section role="status" className="rounded-2xl border border-[#4ade80]/40 bg-[#1c2026] p-6">
    <h2 className="text-xl font-bold">Tu ejercicio ya está en la biblioteca</h2>
  ...
  return <form onSubmit={submit} className="space-y-6">
    <fieldset disabled={saving} className="space-y-5 disabled:opacity-70">
      <label className="block text-sm font-semibold">Nombre del ejercicio<input name="name" required minLength={2} maxLength={120} placeholder="Ej. Sentadilla con banda" className={fieldClass} /></label>
      <label className="block text-sm font-semibold">Descripción<textarea name="description" required minLength={10} maxLength={2000} rows={3} placeholder="Explica en qué consiste el ejercicio." className={fieldClass} /></label>
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="block text-sm font-semibold">Grupo muscular<select name="muscleGroup" required defaultValue="" className={fieldClass}><option value="" disabled>Selecciona un grupo</option>{MUSCLE_GROUPS.map((group) => <option key={group.label}>{group.label}</option>)}</select></label>
        <label className="block text-sm font-semibold">Equipo<input name="equipment" required maxLength={80} placeholder="Ej. Mancuernas o sin equipo" className={fieldClass} /></label>
      </div>
      <label className="block text-sm font-semibold">Cómo hacerlo<textarea name="steps" required maxLength={7500} rows={5} placeholder={"Colócate en la posición inicial.\nRealiza el movimiento.\nVuelve a la posición inicial."} className={fieldClass} /><span className="mt-1 block font-normal text-[#9099a3]">Un paso por línea, hasta 15 pasos de 500 caracteres.</span></label>
      ...
      <label className="flex items-start gap-3 text-sm text-[#9099a3]"><input required type="checkbox" className="mt-1 accent-[#4ade80]" />Tengo permiso para compartir este contenido y entiendo que será visible para toda la comunidad.</label>
      {error && <p role="alert" className="text-sm text-red-400">{error}</p>}
      <button type="submit" disabled={!gif || converting || saving} className="min-h-12 w-full rounded-xl bg-[#4ade80] px-5 font-bold text-[#0d0f12] disabled:opacity-40">{saving ? "Publicando ejercicio…" : "Publicar ejercicio"}</button>
    </fieldset>
  </form>;
}
